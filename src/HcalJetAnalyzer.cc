...
void HcalJetAnalyzer::beginJob(const edm::EventSetup& es)
{
 ...
 edm::Service<TFileService> fs;
  ...
  obs15   =fs->make<TH1F>("phiphi/etaeta ","phiphi/etaeta",100,0,10);
  obs16   =fs->make<TH1F>("EnergyFractionAtMeanPhi","EnergyFractionAtMeanPhi",100,0,10);
...
tree_->Branch("RecoJetEta",   RecoJetEta ,  "RecoJetEta[nJt]/F");
 ...
 tree_->Branch("EtaEtaMoment",EtaEtaMoment,"EtaEtaMoment[nJt]/F"); 
 tree_->Branch("EtaPhiMoment",EtaPhiMoment,"EtaPhiMoment[nJt]/F"); 
 tree_->Branch("PhiPhiMoment",PhiPhiMoment,"PhiPhiMoment[nJt]/F"); 
 
 tree_->Branch("EnergyFractionAtMeanPhi",EnergyFractionAtMeanPhi,"EnergyFractionAtMeanPhi[nJt]/F"); 
  ...
  hPhiIPSA->Fill(track.impactPointTSCP().momentum().phi());
   hEtaIPSA->Fill(track.impactPointTSCP().momentum().eta());
	...
	for(HODigiCollection::const_iterator j=ho->begin();j!=ho->end();j++){
	 ...
	 COND->getHcalCalibrations(digi.id());
...
//xxxxxxxx
EtaEtaMoment[ijt]=(*jets )[ijt].etaetaMoment();
EtaPhiMoment[ijt]=(*jets )[ijt].etaphiMoment ();
PhiPhiMoment[ijt]=(*jets )[ijt].phiphiMoment();
 ...
 /*   if(TriggerType==TRIG_DT){
  ...
  float Energyfixedeta=0.0;
   for(vector<CaloTowerRef>::const_iterator i_Tower=theCaloTowers.begin();i_Tower!=theCaloTowers.end();i_Tower++){
    ...
    if(Fixeta==0)Fixeta=(*i_Tower)->eta();
    if((*i_Tower)->eta()==Fixeta)Energyfixedeta+=(*i_Tower)->energy();
    
     }
     if((*jets )[ijt].energy()>0)EnergyFractionAtMeanPhi[ijt]=Energyfixedeta/(*jets )[ijt].energy();
     obs16->Fill(EnergyFractionAtMeanPhi[ijt]);
...
if(EtaEtaMoment[ijt]>0)obs15->Fill(PhiPhiMoment[ijt]/EtaEtaMoment[ijt]);
